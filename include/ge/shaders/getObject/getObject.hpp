...
namespace ge {
    inline GLuint initShaderGetObject(void)
    {
        std::string vertexSource = ge::loadShaderSource("include/ge/shaders/getObject/vertex.glsl");
        const char *vertexShaderSrc = vertexSource.c_str();

        std::string fragmentSource = ge::loadShaderSource("include/ge/shaders/getObject/fragment.glsl");
